    ...
    for s in range(spin):
        ...
        lib.dot(
            np.ascontiguousarray(
                np.swapaxes(Li_n.reshape((nL, nlo, nemb)), 1, 2).reshape(
                    (nL * nemb, nlo)
                )
            ),
            basis[s, ki].conj(),
            ...
            for j, kptj in enumerate(kpts):
                ...
                if is_kconserv:
                    ...
                    for LpqR, LpqI, sign in mydf.sr_loop(
                        [kpti, kptj], max_memory=max_memory, compact=False
                    ):
                        Lpq = (LpqR + LpqI * 1.0j).reshape(-1, nao, nao)
                        # Lij = _Lpq_to_Lij(Lpq, C_ao_lo, i, j)[0]
                        Lij_emb_pq.append(
                            _Lij_to_Lmn(Lpq, ao2eo, i, j).transpose(1, 0, 2, 3)
                        )
