...
def make_xsf(cell, struc="structure"):
    ...
    atoms_cart = np.asarray(
        [(np.asarray(atom[1]) * param.BOHR).tolist() for atom in cell._atom]
    )
...
def plot_wf(
    w90, rotate_mat=None, outfile="MLWF", supercell=[1, 1, 1], grid=[50, 50, 50]
):
    """
        ...
        with open(outfile + "-" + str(wf_id) + ".xsf", "w") as f:
            ...
            for row in range(3):
                f.write(
                    "%10.7f  %10.7f  %10.7f\n"
                    % (
                        w90.real_lattice_loc[row, 0],
                        w90.real_lattice_loc[row, 1],
...
def plot_mo_gamma(mf, outfile="mo", grid=[50, 50, 50]):
    ...
    grids_coor, weights = pywannier90.periodic_grid(
        mf.mol, grid, supercell=[1, 1, 1], order="C"
    )
