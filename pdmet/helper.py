...
def get_occ_rhf(nelec, mo_energy_kpts):
    ...
    nkpts = len(mo_energy_kpts)  # noqa: F841
    nocc = nelec // 2
...
def get_occ_rohf(nelec, mo_energy_kpts):
    ...
    if hasattr(mo_energy_kpts[0], "mo_ea"):
        ...
        mo_eb_kpts = [x.mo_eb for x in mo_energy_kpts]  # noqa: F841
    else:
        mo_ea_kpts = mo_eb_kpts = mo_energy_kpts  # noqa: F841
...
def irred_kmesh(kpts):
    ...
    for i in range(kpts.shape[0] - 1):
        for j in range(i + 1):
            if abs(kpts[i + 1] + kpts[j]).sum() < 1.0e-10:
                sym_id[i + 1] = sym_id[j]
                break
    kpts_irred, sym_counts = np.unique(sym_id, return_counts=True)
    nkpts_irred = kpts_irred.size  # noqa: F841
