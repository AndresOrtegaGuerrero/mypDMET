...
def get_bath_using_RHF_1RDM(
    ...
    assert (
        np.linalg.norm(np.dot(emb_core_orbs.T, emb_core_orbs) - np.identity(Nlo))
        < 1e-12
    ), "WARNING: The embedding orbitals is not orthogonal"

    return emb_orbs, core_orbs, Nelec, Nbath


def get_bath_using_gamma_RHF_1RDM(supercell_1RDM, imp_indices=None, threshold=1.0e-10):
    """
    Construct the RHF bath using the 1RDM for reference unit cell
    ref:
    ...
    assert np.linalg.norm(np.dot(emb_orbs.T, emb_orbs) - np.identity(Nemb)) < 1e-12, (
        "WARNING: The embedding orbitals is not orthogonal"
    )
