...
class DMRG:
    ...
    def __init__(self, mf, nroots):
        ...
        self.g2e = ao2mo.incore.general(
            mf._eri, (mf.mo_coeff,) * 4, compact=False
        ).reshape(self.norb, self.norb, self.norb, self.norb)
    ...
    def run(self):
            ...
            for orb2 in range(self.norb):
                ...
                for orb3 in range(self.norb):
                    for orb4 in range(self.norb):
                        HamCheMPS2.setVmat(
                            orb1, orb2, orb3, orb4, self.g2e[orb1, orb3, orb2, orb4]
        ...
        RDM2 = np.zeros(
            [self.norb, self.norb, self.norb, self.norb], dtype=ctypes.c_double
        )
        for orb1 in range(self.norb):
            for orb2 in range(self.norb):
                for orb3 in range(self.norb):
