...
class QCsolvers:
    def __init__(
        ...
        if self.solver == "CASCI":
            ...
            self._convergence = [1.0e-4, 1.0e-5, 1.0e-6, 1.0e-8]
            self.noise = 0.03
            self.max_sweep = 100
            self._davidson_tol = [1.0e-3, 1.0e-4, 1.0e-5, 1.0e-6]
    ...
    def HF(self):
        ...
        DMloc = np.dot(
            np.dot(self.mf.mo_coeff, np.diag(self.mf.mo_occ)), self.mf.mo_coeff.T
        )
        ...
        if self.mol.spin == 0 and not self._is_KROHF:
            ImpurityEnergy = 0.5 * lib.einsum(
                "ij,ij->", RDM1[:Nimp, :], self.FOCK[:Nimp, :] + self.OEI[:Nimp, :]
            ) + 0.5 * lib.einsum("ij,ij->", RDM1[:Nimp, :], JK[:Nimp, :])
        else:
            ImpurityEnergy_a = 0.5 * lib.einsum(
                "ij,ij->", RDM1[0][:Nimp, :], self.FOCK[:Nimp, :] + self.OEI[:Nimp, :]
            ) + 0.5 * lib.einsum("ij,ij->", RDM1[0][:Nimp, :], JK[0][:Nimp, :])
            ImpurityEnergy_b = 0.5 * lib.einsum(
                "ij,ij->", RDM1[1][:Nimp, :], self.FOCK[:Nimp, :] + self.OEI[:Nimp, :]
    ...
    def MP2(self):
        ...
        ImpurityEnergy = (
            ...
            * lib.einsum(
            ...
            + 0.125
            * lib.einsum("ijkl,ijkl->", RDM2[:Nimp, :, :, :], self.TEI[:Nimp, :, :, :])
            + 0.125
            * lib.einsum("ijkl,ijkl->", RDM2[:, :Nimp, :, :], self.TEI[:, :Nimp, :, :])
            + 0.125
            * lib.einsum("ijkl,ijkl->", RDM2[:, :, :Nimp, :], self.TEI[:, :, :Nimp, :])
    ...
    def convergence(self, value):
        converg = [value * 1.0e4, value * 1.0e3, value * 1.0e2, value]
    ...
    def davidson_tol(self, value):
        dav_tol = [value * 1.0e3, value * 1.0e2, value * 1.0e1, value]
    ...
    def DMRG(self):
        ...
        # CheMPS2 calculation
        Initializer = PyCheMPS2.PyInitialize()  # noqa: F821
        Initializer.Init()
        Group = 0
        orbirreps = np.zeros([Norb], dtype=ctypes.c_int)
        HamCheMPS2 = PyCheMPS2.PyHamiltonian(Norb, Group, orbirreps)  # noqa: F821
            ...
            for orb2 in range(Norb):
                ...
                for orb3 in range(Norb):
                    for orb4 in range(Norb):
                        HamCheMPS2.setVmat(
                            orb1, orb2, orb3, orb4, self.TEI[orb1, orb3, orb2, orb4]
                        )  # From chemist to physics notation

        assert self.Nel % 2 == 0
        TwoS = self.twoS
        Irrep = 0

        if not self.CheMPS2print:
            sys.stdout.flush()
        ...
        Prob = PyCheMPS2.PyProblem(HamCheMPS2, TwoS, self.Nel, Irrep)  # noqa: F821
        OptScheme = PyCheMPS2.PyConvergenceScheme(4)  # 3 instructions # noqa: F821
        ...
        OptScheme.set_instruction(
            0, self._D[0], self._convergence[0], 5, self.noise, self._davidson_tol[0]
        )
        OptScheme.set_instruction(
            1, self._D[1], self._convergence[1], 5, self.noise, self._davidson_tol[1]
        )
        OptScheme.set_instruction(
            2, self._D[2], self._convergence[2], 5, self.noise, self._davidson_tol[2]
        )
        OptScheme.set_instruction(
            3,
            self._D[3],
            ...
            self.max_sweep,
            0.00,
            self._davidson_tol[3],
        )  # Last instruction a few iterations without noise

        theDMRG = PyCheMPS2.PyDMRG(Prob, OptScheme)  # noqa: F821
        ...
        for orb1 in range(Norb):
            for orb2 in range(Norb):
                for orb3 in range(Norb):
                    for orb4 in range(Norb):
                        RDM2[orb1, orb3, orb2, orb4] = theDMRG.get2DMA(
                            orb1, orb2, orb3, orb4
        ...
        if self.nroots == 1:
            ImpurityEnergy = (
                ...
                * lib.einsum(
                ...
                + 0.125
                * lib.einsum(
                    "ijkl,ijkl->", RDM2[:Nimp, :, :, :], self.TEI[:Nimp, :, :, :]
                )
                + 0.125
                * lib.einsum(
                    "ijkl,ijkl->", RDM2[:, :Nimp, :, :], self.TEI[:, :Nimp, :, :]
            ...
            for i in range(self.nroots):
                ...
                print(
                    "       Root %d: E(Solver) = %12.8f  E(Imp) = %12.8f  <S^2> = %8.6f"
                    % (i, EDMRG[i], Imp_e, self.SS)
                )
                e_cell.append(Imp_e)
    ...
    def FCI(self):
        ...
        if self.e_shift is None:
            self.fs = fci.FCI(self.mf, self.mf.mo_coeff)
        else:
            self.fs = fci.addons.fix_spin_(
                fci.FCI(self.mf, self.mf.mo_coeff), self.e_shift
            )
    ...
    def SHCI(self):
        ...
        if self.nroots == 1:
            ...
            for i, vec in enumerate(fcivec):
                ...
                print(
                    "       Root %d: E(Solver) = %12.8f  E(Imp) = %12.8f  <S^2> = %8.6f"
                    % (i, e_cell[i], Imp_e, SS)  # Check if e_cell[i] is correct
                )
            RDM1 = lib.einsum("i,ijk->jk", self.state_percent, RDM1)
            e_cell = lib.einsum("i,i->", self.state_percent, e_cell)
            self.SS = tot_SS / self.nroots
    ...
    def CASCI(self, solver="FCI", nevpt2_roots=None, nevpt2_nroots=10):
        ...
        # Define FCI solver
        if solver == "CheMPS2":
            self.mc.fcisolver = dmrgscf.CheMPS2(self.mol)  # noqa: F821
        ...
        if self.nroots == 1:
            ...
            RDM1_mo, RDM2_mo = self.mc.fcisolver.make_rdm12(
                civec, self.Norb, self.mol.nelec
            )
            ...
            casdm1_mo, casdm2_mo = self.mc.fcisolver.make_rdm12(
                self.mc.ci, cas_norb, self.mc.nelecas
            )  # in CAS(MO) space
            ...
            for i, civec in enumerate(fcivec):
                ...
                active_MO = self.mc.mo_coeff[:, core_norb : core_norb + cas_norb]
                casdm1_mo, casdm2_mo = self.mc.fcisolver.make_rdm12(
                    civec, cas_norb, self.mc.nelecas
                )  # in CAS(MO) space
        ...
        if nevpt2_roots is not None:
            ...
            for root in nevpt2_roots:
                SS = mc_CASCI.fcisolver.spin_square(
                    fcivec[root], cas_norb, self.mc.nelecas
                )[0]
    ...
    def CASSCF(
        ...
        if state_specific_ is not None and state_average_ is None:
            ...
            state_specific_ is None
            and state_average_ is not None
            and state_average_mix_ is None
        ):
        ...
        if solver == "CheMPS2":
            ...
            solver == "FCI" and self.e_shift is not None and state_average_mix_ is None
        ):
            target_SS = 0.5 * self.twoS * (0.5 * self.twoS + 1)
            self.mc.fix_spin_(shift=self.e_shift, ss=target_SS)
        ...
        if self.nroots == 1 or state_specific_ is not None:
            ...
            e_cell = self.kmf_ecore + ImpurityEnergy
            print(
                "       State %d: E(Solver) = %12.8f  E(Imp) = %12.8f  <S^2> = %8.6f"
                % (state_id, e_tot, ImpurityEnergy, self.SS)
            )
        elif state_average_ is not None:
            tot_SS = 0
            RDM1 = []
            e_cell = []
            rdm1s, rdm2s = self.mc.fcisolver.states_make_rdm12(
                fcivec, cas_norb, self.mc.nelecas
            )
            SSs, spin_multiplicities = self.mc.fcisolver.states_spin_square(
                fcivec, cas_norb, self.mc.nelecas
            )

            for i in range(len(weights)):
                SS, spin_multiplicity = SSs[i], spin_multiplicities[i]  # noqa: F841
                ...
                if True:
                    ...
                    coredm2 = np.zeros(
                        [self.Norb, self.Norb, self.Norb, self.Norb], dtype=np.float64
                    )  # TODO: this is impractical for the big embedding space. Lots of memory
                    coredm2 += lib.einsum("pq,rs-> pqrs", coredm1, coredm1)
                    coredm2 -= 0.5 * lib.einsum("ps,rq-> pqrs", coredm1, coredm1)
                ...
                if state_average_ is not None:
                    print(
                        "       State %d (%5.3f): E(Solver) = %12.8f  E(Imp) = %12.8f  <S^2> = %8.6f"
                        % (i, weights[i], e_tot[i], Imp_e, SS)
                    )
        ...
        if nevpt2_roots is not None:
            ...
            print("=====================================")
            if len(nevpt2_roots) > len(fcivec):
                nevpt2_roots = np.arange(len(fcivec))
            for root in nevpt2_roots:
                ci = fcivec[root]
                ...
                print(
                    "== State {0:d}: {1:2.4f}|{2:s},{3:s}> + {4:2.4f}|{5:s},{6:s}> + {7:2.4f}|{8:s},{9:s}> + {10:2.4f}|{11:s},{12:s}>".format(
                        root,
                        c1,
                        bin(stra_1st)[2:],
                        bin(strb_1st)[2:],
                ...
                t_dm1 = mc_CASCI.fcisolver.trans_rdm1(
                    ground_state, ci, mc_CASCI.ncas, mc_CASCI.nelecas
                )
                # transform density matrix to EO representation
                orbcas = mc_CASCI.mo_coeff[
                    :, mc_CASCI.ncore : mc_CASCI.ncore + mc_CASCI.ncas
                ]
    ...
    from pyscf.mcpdft.otfnal import transfnal, ftransfnal
    from pyscf.mcpdft.mcpdft import energy_tot
    from pyscf.mcpdft import mcpdft
    ...
    def CASPDFT(
        ...
        self.mf.__init__(self.mol)
        self.mf.get_hcore = lambda *args: FOCKcopy
        self.mf.get_ovlp = lambda *args: np.eye(
            self.Norb
        )  # just a diagonal matrix with the correct dimensions
        self.mf._eri = ao2mo.restore(
            8, self.TEI, self.Norb
        )  # not same in two different calculations; which basis is this in?
        self.mf.scf(self.DMguess)
        DMloc = np.dot(
            np.dot(self.mf.mo_coeff, np.diag(self.mf.mo_occ)), self.mf.mo_coeff.T
        )
        DM1loc = self.mf.make_rdm1()  # noqa: F841
        if self.mf.converged is False:
            self.mf.newton().kernel(dm0=DMloc)
            DMloc = np.dot(
                np.dot(self.mf.mo_coeff, np.diag(self.mf.mo_occ)), self.mf.mo_coeff.T
        ...
        if self.nroots == 1 or state_specific_ is not None:
            civec = fcivec
            self.SS, spin_multiplicity = mcscf.spin_square(self.mc)  # noqa: F841
            ###### Get RDM1 + RDM2 #####
            core_norb = self.mc.ncore
            core_MO = self.mc.mo_coeff[:, :core_norb]
            active_MO = self.mc.mo_coeff[:, core_norb : core_norb + cas_norb]
            ...
            rdm1spin_sep_a, rdm1spin_sep_b = self.mc.fcisolver.make_rdm1s(
                fcivec, cas_norb, self.mc.nelecas
            )  # get spin-separated RDMs
            rdm1spin_sep_ = [rdm1spin_sep_a, rdm1spin_sep_b]  # noqa: F841
            ...
            ao_basis_casdm1sa = lib.einsum(
                "im,mn,jn->ij", self.ao2eo[0].real, casdm1sa, self.ao2eo[0].real.conj()
            )
            ao_basis_casdm1sb = lib.einsum(
                "im,mn,jn->ij", self.ao2eo[0].real, casdm1sb, self.ao2eo[0].real.conj()
            )
            ao_basis_casdm1s = np.asarray([ao_basis_casdm1sa, ao_basis_casdm1sb])  # noqa: F841
            RDM1 = coredm1 + casdm1
            RDM1Sa = coredm1 / 2 + casdm1sa
            RDM1Sb = coredm1 / 2 + casdm1sb
            RDM1s = [RDM1Sa, RDM1Sb]  # noqa: F841
            nelectrons = np.trace(RDM1[:, :])  # noqa: F841
            casdm1s = [casdm1sa_mo, casdm1sb_mo]
            ao_basis_RDM1 = lib.einsum(  # noqa: F841
                "im,mn,jn->ij", self.ao2eo[0].real, RDM1, self.ao2eo[0].conj().real
            )
            ao_basis_RDM1sa = lib.einsum(
                "Rim,mn,jn->Rij", self.ao2eo.real, RDM1Sa, self.ao2eo[0].conj().real
            )
            ao_basis_RDM1sb = lib.einsum(
                "Rim,mn,jn->Rij", self.ao2eo.real, RDM1Sb, self.ao2eo[0].conj().real
            )
            ao_basis_RDM1s = [ao_basis_RDM1sa, ao_basis_RDM1sb]  # noqa: F841
            ...
            self.loc_OEH_kpts, self.loc_1RDM_kpts, self.loc_1RDM_R0 = (
                self.local.make_loc_1RDM(
                    0.0, self.mask4Gamma, OEH_type=self.OEH_type, dft_HF=None
                )
            )
            emb_core_1RDM_for_mcpdft = self.local.make_emb_space_RDM(
                self.loc_1RDM_R0, self.emb_orbs, self.core_orbs, self.emb_core_orbs
            )
            ...
            ao_basis_emb_core_1RDM_for_mcpdft = self.local.loc_kpts_to_emb_trial_2(
                self.loc_1RDM_R0_modified,
                self.emb_orbs,
                self.core_orbs,
                self.emb_core_orbs,
                emb_core_1RDM_for_mcpdft,
            )
            ao_basis_emb_core_1RDM_for_mcpdft_1sa = self.local.loc_kpts_to_emb_trial_2(
                self.loc_1RDM_R0_modified,
                self.emb_orbs,
                self.core_orbs,
                self.emb_core_orbs,
                emb_core_1RDM_for_mcpdft1sa,
            )
            ao_basis_emb_core_1RDM_for_mcpdft_1sb = self.local.loc_kpts_to_emb_trial_2(
                self.loc_1RDM_R0_modified,
                self.emb_orbs,
                self.core_orbs,
                self.emb_core_orbs,
                emb_core_1RDM_for_mcpdft1sb,
            )
            self.loc_1RDM_R0_modified[0][: self.Norb, : self.Norb] = RDM1
            self.loc_1RDM_R0_modified_ao_basis = np.asarray(
                [ao_basis_emb_core_1RDM_for_mcpdft]
            )
            self.loc_1RDM_R0_modified_ao_basis_1sa = np.asarray(
                [ao_basis_emb_core_1RDM_for_mcpdft_1sa]
            )
            self.loc_1RDM_R0_modified_ao_basis_1sb = np.asarray(
                [ao_basis_emb_core_1RDM_for_mcpdft_1sb]
            )
            self.loc_1RDM_R0_modified_ao_basis_1s = [
                self.loc_1RDM_R0_modified_ao_basis_1sa,
                self.loc_1RDM_R0_modified_ao_basis_1sb,
            ]
            e_pdft = get_dmet_pdft(
                self.mc,
                RDM1,
                "tPBE",
                casdm1s,
                ...
                cell,
                mc_dup=mo,
                ao_basis_RDM1s=self.loc_1RDM_R0_modified_ao_basis_1s,
                ao2eo=self.ao2eo,
                kmf=self.kmf,
            )
            print("The MC-PDFT energy is ", e_pdft)
        elif state_average_ is not None:  # still under development
            nroots = self.mc.fcisolver.nroots = self.nroots  # noqa: F841
            civec = fcivec
            tot_SS = 0
            RDM1 = []
            e_cell = []
            rdm1spin_sep_a, rdm1spin_sep_b = self.mc.fcisolver.states_make_rdm1s(
                fcivec, cas_norb, self.mc.nelecas
            )
            rdm1spin_sep_ = [rdm1spin_sep_a[0], rdm1spin_sep_b[0]]  # noqa: F841
            rdm1s, rdm2s = self.mc.fcisolver.states_make_rdm12(
                fcivec, cas_norb, self.mc.nelecas
            )  # This is rdm1 and rdm2, not 1s and 2s
            # dm1s = np.asarray ( self.mc.make_rdm1s() )
            SSs, spin_multiplicities = self.mc.fcisolver.states_spin_square(
                fcivec, cas_norb, self.mc.nelecas
            )
            # print("Am I here? -1167")
            casdm1s_append = []  # noqa: F841
            for i in range(len(weights)):
                weights = state_average_
                if "FakeCISolver" not in str(self.mc.fcisolver):
                    self.mc = self.mc.state_average_(weights)
                ...
                ao_basis_emb_core_1RDM_for_mcpdft_1sa = (
                    self.local.loc_kpts_to_emb_trial_2(
                        self.loc_1RDM_R0_modified,
                        self.emb_orbs,
                        self.core_orbs,
                        self.emb_core_orbs,
                ...
                casdm1s = [casdm1sa_mo, casdm1sb_mo]
                rdm1spin_sep = [rdm1spin_sep_a[i], rdm1spin_sep_b[i]]  # noqa: F841
                epdft = get_dmet_sapdft(
                    self.mc,
                    rdm1,
                    casdm1s,
                    casdm1_mo,
...
def get_E_ot(
    ...
    ncore, ncas, _ = mc.ncore, mc.ncas, mc.nelecas  # ncore, ncas, nelecas
    nocc = ncore + ncas
    mo_cas = mo_coeff[:, ncore:nocc]
    ao2eo_inv = np.linalg.pinv(ao2eo[0])
    mo_cas_ao2eo = lib.einsum("ap,pq->aq", ao2eo[0].real, mo_cas)
    moH_cas = mo_cas.conj().T  # noqa: F841
    mo_core = mo_coeff[:, :ncore]
    moH_core = mo_core.conj().T  # noqa: F841
    print("mo_cas", mo_cas)
    print("ao2eo_inv", ao2eo_inv.real)
    print(
        "lib.einsum('ap,pq->aq',  ao2eo[0].real, mo_cas )",
        lib.einsum("ap,pq->aq", ao2eo_inv.real, mo_cas),
    )
    ...
    spin = abs(mc.nelecas[0] - mc.nelecas[1])
    t0 = logger.timer(ot, "rdms", *t0)
    omega, alpha, hyb = ot._numint.rsh_and_hybrid_coeff(ot.otxc, spin=spin)  # noqa: F841
    spin = abs(mc.nelecas[0] - mc.nelecas[1])
    ...
    for ao, mask, weight, coords in ni.block_loop(
        ...
        cascm2 = _dms.dm2_cumulant(
            casdm2, casdm1s
        )  # Check the exact implementations here
    ...
    if isinstance(mc, StateAverageMCSCFSolver):
        ...
        if isinstance(mc.fcisolver, StateAverageMixFCISolver):
            ...
            for s in mc.fcisolver.fcisolvers:
                nelec_root.extend(
                    [
                        mc.fcisolver._get_nelec(s, mc.nelecas),
                    ]
                    * s.nroots
                )
        for ci_i, ei_mcscf, nelec in zip(ci, e_mcscf, nelec_root):
            row = _get_e_decomp(
                mc, ot, mo_coeff, ci_i, ei_mcscf, e_nuc, h, xfnal, cfnal, nelec
            )
            e_core.append(row[0])
            e_coul.append(row[1])
            e_otx.append(row[2])
        ...
        e_core, e_coul, e_otx, e_otc, e_wfnxc = _get_e_decomp(
            mc, ot, mo_coeff, ci, e_mcscf, e_nuc, h, xfnal, cfnal, mc.nelecas
        )
    print("e_nuc", e_nuc)
    print("e_core", e_core)
    print("e_coul", e_coul)
...
def SAMCPDFT(
    ...
    spin = abs(mc.nelecas[0] - mc.nelecas[1])
    t0 = logger.timer(ot, "rdms", *t0)
    _, _, hyb = ot._numint.rsh_and_hybrid_coeff(
        ot.otxc, spin=spin
    )  # omega, alpha , hyb
    Vnn = cell.energy_nuc()
    hyb_x, hyb_c = hyb
    ...
    if log.verbose >= logger.DEBUG or abs(hyb_x) > 1e-10 or abs(hyb_c) > 1e-10:
        E_x = (
            -(
                np.tensordot(vk[0], rdm1spin_sep_a[0])
                + np.tensordot(vk[1], rdm1spin_sep_b[1])
            )
            / 2
        )
    else:
...
def sa_get_E_ot(
    ...
    if ot is None:
        ot = mc.otfnal
    if mo_coeff is None:
        mo_coeff = mc.mo_coeff  # 2 change
    ncore, ncas, _ = mc.ncore, mc.ncas, mc.nelecas  # ncore, ncas, nelecas
    nocc = ncore + ncas
    mo_cas = mo_coeff[:, ncore:nocc]
    mo_cas_ao2eo = lib.einsum("ap,pq->aq", ao2eo[0].real, mo_cas)
    mo_cas2 = mc.mo_coeff[:, ncore:nocc]  # noqa: F841
    moH_cas = mo_cas.conj().T  # noqa: F841
    mo_core = mo_coeff[:, :ncore]
    moH_core = mo_core.conj().T  # noqa: F841
    t0 = (time.clock(), time.time())
