...
def test_cost_function():
    ...
    assert np.linalg.norm(numerical_deriv - analytical_deriv) < 5.0e-5

    """Test dmet.rdm_diff_grad function"""
    loc_OEH_kpts = pdmet.get_loc_OEH_kpts(umat0)
    _, _ = pdmet.local.make_loc_1RDM(loc_OEH_kpts)
    ...
    assert np.linalg.norm(numerical_rdm_diff_grad - analytical_rdm_diff_grad) < 5.0e-5
    ...
    assert np.linalg.norm(numerical_CF_grad - analytical_CF_grad) < 5.0e-5
